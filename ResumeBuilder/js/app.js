...
const S = {
    ...
    sections: [],
    settings: {layout:'classic',accentColor:'#2563eb',fontColor:'#1f2937',fonts:{header:'Inter',sectionTitle:'Inter',body:'Inter',contact:'Inter',skills:'Inter',declaration:'Inter'},fontSizes:{header:'medium',sectionTitle:'medium',body:'medium'},spacing:'normal',theme:'light'},
...
function bindTheme() {
    ...
    $$('.sw.fc').forEach(b=>b.addEventListener('click',()=>{
        $$('.sw.fc').forEach(x=>x.classList.remove('active'));
        b.classList.add('active');
        S.settings.fontColor=b.dataset.fc;
        applyAccent();render();
    }));
...
function applyAccent() {
    const a=S.settings.accentColor,f=S.settings.fontColor;
...
function bindHexPicker() {
    $$('.cpk-wrap[data-picker]').forEach(wrap => {
        wrap.addEventListener('click', e => {
            ...
            if (!isOpen) {
                ...
                const currentColor = type === 'accent' ? S.settings.accentColor : S.settings.fontColor;
                const inp = $('#' + type + 'Hex');
                const pv = $('#' + type + 'Preview');
                inp.value = currentColor.replace('#', '').toUpperCase();
                pv.style.background = currentColor;
    ...
    ['accent', 'font'].forEach(type => {
        ...
        input.addEventListener('keydown', function(e) {
            if (e.key === 'Enter') { e.preventDefault(); applyHexColor(type); }
        });
    });
...
function bindFontPanel() {
    ...
    $('#fontOpen').addEventListener('click',()=>{p.classList.add('show');o.classList.add('show')});
    const cl=()=>{p.classList.remove('show');o.classList.remove('show')};
...
function bindZoom() {
    ...
    try{new ResizeObserver(()=>{if(mode==='fit')apply()}).observe(pvBd)}catch(e){}
    ...
    $('#zFit').addEventListener('click',()=>{mode='fit';apply()});
...
function bindResponsive() {
    ...
    // Move the floating FAB inside .mg so the CSS selector .mg.vp .back-edit-fab works
...
function bindHeader() {
    ...
    const reset=()=>{
        ...
        applyPhotoShape();renderEditor();render();toast('Reset done','inf');
    };
    const pairs=[
...
function applyFontsToContent(rc) {
    const f=S.settings.fonts,fs=S.settings.fontSizes;
...
function rDecl(){
    ...
    h+=`<div class="rdclt">Declaration</div>`;
...
function buildOffscreenStyles(prefix) {
    ...
    const fc = S.settings.fontColor;
    const f = S.settings.fonts;
    return `
    .${prefix} * { box-sizing:border-box; margin:0; padding:0; }
    .${prefix} { width:794px; height:1123px; background:#fff; font-family:'${f.body}',sans-serif; color:${fc}; }
    ...
    .${prefix} .layout-elegant .rn { font-family:'Playfair Display',serif; font-size:28px; font-weight:700; letter-spacing:1px; margin-bottom:4px; color:${fc}; }
    ...
    .${prefix} .layout-elegant .rct { display:flex; justify-content:center; flex-wrap:wrap; gap:5px 14px; font-size:9.5px; color:#6b7280; font-family:'${f.contact}',sans-serif; }
    ...
    .${prefix} .layout-creative .rn { font-size:26px; font-weight:900; margin-bottom:2px; color:${fc}; font-family:'${f.header}',sans-serif; }
    .${prefix} .layout-creative .rt { font-size:12px; color:${ac}; font-weight:600; margin-bottom:10px; font-family:'${f.header}',sans-serif; }
    .${prefix} .layout-creative .rct { display:flex; flex-wrap:wrap; gap:6px 14px; font-size:10px; color:#6b7280; font-family:'${f.contact}',sans-serif; }
    ...
    .${prefix} .rst { font-size:11.5px; font-weight:800; color:${ac}; text-transform:uppercase; letter-spacing:1.2px; margin-bottom:8px; padding-bottom:3px; border-bottom:2px solid ${acl}; display:flex; align-items:center; gap:5px; font-family:'${f.sectionTitle}',sans-serif; }
    .${prefix} .rst i { font-size:10px; opacity:.6; }
    .${prefix} .rtx { font-size:10px; color:${fc}; line-height:1.65; white-space:pre-line; opacity:.85; font-family:'${f.body}',sans-serif; }
    ...
    .${prefix} .ret { font-size:11px; font-weight:700; color:${fc}; font-family:'${f.body}',sans-serif; }
    ...
    .${prefix} .redc { font-size:9.5px; color:${fc}; line-height:1.55; white-space:pre-line; opacity:.85; font-family:'${f.body}',sans-serif; }
    .${prefix} .relk { font-size:9px; color:${ac}; text-decoration:none; }
    .${prefix} .rsg { display:flex; flex-wrap:wrap; gap:5px; }
    .${prefix} .rsk { padding:2px 8px; background:${acll}; color:${ac}; border-radius:3px; font-size:9.5px; font-weight:600; border:1px solid ${acl}; font-family:'${f.skills}',sans-serif; }
    ...
    .${prefix} .rcn { font-size:10.5px; font-weight:700; color:${fc}; }
    ...
    .${prefix} .rln { font-size:10px; font-weight:700; color:${fc}; }
    .${prefix} .rll { font-size:9px; color:#6b7280; }
    .${prefix} .rints { display:flex; flex-wrap:wrap; gap:5px; }
    .${prefix} .rint { padding:2px 9px; background:#f3f4f6; border-radius:100px; font-size:9.5px; color:#4b5563; font-weight:500; }
    ...
    .${prefix} .rdclt { font-size:10.5px; font-weight:800; color:${ac}; text-transform:uppercase; letter-spacing:1.2px; margin-bottom:8px; padding-bottom:3px; border-bottom:2px solid ${acl}; font-family:'${f.sectionTitle}',sans-serif; }
    .${prefix} .rdctx { font-size:9.5px; color:${fc}; line-height:1.65; font-style:italic; margin-bottom:16px; opacity:.85; font-family:'${f.declaration}',sans-serif; }
    .${prefix} .rdcf { display:flex; justify-content:space-between; align-items:flex-end; gap:14px; }
    .${prefix} .rdcm { font-size:9px; color:#6b7280; flex-shrink:0; font-family:'${f.declaration}',sans-serif; }
    ...
    .${prefix} .rdcs .sign { font-size:10px; font-weight:700; color:${fc}; font-family:'${f.declaration}',sans-serif; }`;
}
...
html, body { width:210mm; height:297mm; background:#fff; font-family:'${f.body}', sans-serif; }
.rpage { width:210mm; height:297mm; background:#fff; position:relative; overflow:hidden; }
.rcontent { width:100%; height:100%; display:flex; flex-direction:column; color:${fc}; line-height:1.5; font-size:12px; }

.layout-classic .rh { background:linear-gradient(135deg, ${acd}, ${ac}); color:#fff; padding:12mm 14mm; display:flex; align-items:center; gap:8mm; }
.layout-classic .rph { width:26mm; height:26mm; object-fit:cover; border:2px solid rgba(255,255,255,.25); flex-shrink:0; }
.layout-classic .rn { font-size:22pt; font-weight:800; margin-bottom:1mm; color:#fff; font-family:'${f.header}',sans-serif; }
.layout-classic .rt { font-size:11pt; opacity:.8; margin-bottom:3mm; color:#fff; font-family:'${f.header}',sans-serif; }
.layout-classic .rct { display:flex; flex-wrap:wrap; gap:2mm 4mm; font-size:8pt; opacity:.85; color:#fff; font-family:'${f.contact}',sans-serif; }
.layout-classic .rct span { display:flex; align-items:center; gap:1mm; }
.layout-classic .rbd { padding:9mm 14mm; flex:1; overflow:hidden; }

.layout-modern { flex-direction:row !important; }
.layout-modern .rsb { width:70mm; background:linear-gradient(180deg, ${acd}, ${ac}); color:#fff; padding:12mm 7mm; flex-shrink:0; }
.layout-modern .rsb .rph { width:35mm; height:35mm; object-fit:cover; margin:0 auto 6mm; display:block; border:2px solid rgba(255,255,255,.2); }
.layout-modern .rsb .rn { font-size:14pt; font-weight:800; text-align:center; color:#fff; margin-bottom:1mm; font-family:'${f.header}',sans-serif; }
.layout-modern .rsb .rt { font-size:8pt; text-align:center; opacity:.7; color:#fff; margin-bottom:7mm; font-family:'${f.header}',sans-serif; }
.layout-modern .rsb .sbs { margin-bottom:6mm; }
.layout-modern .rsb .sbt { font-size:7.5pt; font-weight:700; text-transform:uppercase; letter-spacing:1.5px; opacity:.6; margin-bottom:3mm; padding-bottom:1.5mm; border-bottom:1px solid rgba(255,255,255,.15); color:#fff; font-family:'${f.sectionTitle}',sans-serif; }
.layout-modern .rsb .ci { display:flex; align-items:flex-start; gap:1.5mm; font-size:7pt; margin-bottom:1.5mm; opacity:.9; word-break:break-all; color:#fff; font-family:'${f.contact}',sans-serif; }
.layout-modern .rsb .ci i { width:3mm; text-align:center; font-size:7.5pt; opacity:.6; flex-shrink:0; }
.layout-modern .rsb .sbsk { display:inline-block; padding:.5mm 1.5mm; background:rgba(255,255,255,.15); border-radius:1mm; font-size:7pt; margin:0 1mm 1mm 0; color:#fff; font-family:'${f.skills}',sans-serif; }
.layout-modern .rsb .sblg { font-size:7pt; margin-bottom:1mm; opacity:.9; color:#fff; }
.layout-modern .rmn { flex:1; padding:10mm 8mm; display:flex; flex-direction:column; overflow:hidden; }
.layout-modern .rmn .rbdi { flex:1; }

.layout-elegant .rh { text-align:center; padding:14mm 18mm 6mm; border-bottom:2px solid ${ac}; }
.layout-elegant .rph { width:28mm; height:28mm; object-fit:cover; margin:0 auto 4mm; display:block; border:2px solid ${ac}; }
.layout-elegant .rn { font-family:'Playfair Display',serif; font-size:24pt; font-weight:700; letter-spacing:1px; margin-bottom:1mm; color:${fc}; }
.layout-elegant .rt { font-size:9pt; color:${ac}; font-weight:500; letter-spacing:2px; text-transform:uppercase; margin-bottom:4mm; font-family:'${f.header}',sans-serif; }
.layout-elegant .rct { display:flex; justify-content:center; flex-wrap:wrap; gap:1.5mm 4mm; font-size:8pt; color:#6b7280; font-family:'${f.contact}',sans-serif; }
.layout-elegant .rbd { padding:9mm 18mm; flex:1; overflow:hidden; }

.layout-compact .rh { background:${ac}; color:#fff; padding:6mm 11mm; display:flex; align-items:center; gap:6mm; }
.layout-compact .rph { width:18mm; height:18mm; object-fit:cover; border:1.5px solid rgba(255,255,255,.3); flex-shrink:0; }
.layout-compact .rhi { flex:1; }
.layout-compact .rn { font-size:16pt; font-weight:800; color:#fff; font-family:'${f.header}',sans-serif; }
.layout-compact .rt { font-size:8pt; opacity:.8; color:#fff; font-family:'${f.header}',sans-serif; }
.layout-compact .rct { display:flex; flex-wrap:wrap; gap:1.5mm 3mm; font-size:7pt; opacity:.85; margin-top:1mm; color:#fff; font-family:'${f.contact}',sans-serif; }
.layout-compact .rct span { display:flex; align-items:center; gap:1mm; }
.layout-compact .rbd { padding:7mm 11mm; display:grid; grid-template-columns:1fr 1fr; gap:6mm 8mm; flex:1; overflow:hidden; }
.layout-compact .rsc.fw { grid-column:1/-1; }

.layout-creative { position:relative; }
.layout-creative::before { content:''; position:absolute; left:0; top:0; bottom:0; width:2mm; background:linear-gradient(180deg, ${ac}, ${acd}); }
.layout-creative .rh { padding:12mm 13mm 9mm 15mm; display:flex; align-items:flex-end; gap:7mm; border-bottom:.3mm solid #e5e7eb; }
.layout-creative .rph { width:26mm; height:26mm; object-fit:cover; box-shadow:0 1mm 3mm rgba(0,0,0,.15); flex-shrink:0; }
.layout-creative .rn { font-size:22pt; font-weight:900; margin-bottom:1mm; color:${fc}; font-family:'${f.header}',sans-serif; }
.layout-creative .rt { font-size:10pt; color:${ac}; font-weight:600; margin-bottom:3mm; font-family:'${f.header}',sans-serif; }
.layout-creative .rct { display:flex; flex-wrap:wrap; gap:2mm 4mm; font-size:8pt; color:#6b7280; font-family:'${f.contact}',sans-serif; }
.layout-creative .rct span { display:flex; align-items:center; gap:1mm; }
.layout-creative .rct i { color:${ac}; }
.layout-creative .rbd { padding:8mm 13mm 9mm 15mm; flex:1; overflow:hidden; }

.rsc { margin-bottom:6mm; }
.rsc:last-child { margin-bottom:0; }
.rst { font-size:9.5pt; font-weight:800; color:${ac}; text-transform:uppercase; letter-spacing:1.2px; margin-bottom:2.5mm; padding-bottom:1mm; border-bottom:.5mm solid ${acl}; display:flex; align-items:center; gap:1.5mm; font-family:'${f.sectionTitle}',sans-serif; }
.rst i { font-size:8pt; opacity:.6; }
.rtx { font-size:8.5pt; color:${fc}; line-height:1.65; white-space:pre-line; opacity:.85; font-family:'${f.body}',sans-serif; }
.ren { margin-bottom:4mm; }
.ren:last-child { margin-bottom:0; }
.reh { display:flex; justify-content:space-between; align-items:baseline; gap:2mm; margin-bottom:.3mm; }
.ret { font-size:9pt; font-weight:700; color:${fc}; font-family:'${f.body}',sans-serif; }
.red { font-size:7.5pt; color:#9ca3af; white-space:nowrap; font-weight:500; }
.res { font-size:8.5pt; color:${ac}; font-weight:600; margin-bottom:.5mm; font-family:'${f.body}',sans-serif; }
.redc { font-size:8pt; color:${fc}; line-height:1.55; white-space:pre-line; opacity:.85; font-family:'${f.body}',sans-serif; }
.relk { font-size:7.5pt; color:${ac}; text-decoration:none; }
.rsg { display:flex; flex-wrap:wrap; gap:1.5mm; }
.rsk { padding:.5mm 2mm; background:${acll}; color:${ac}; border-radius:1mm; font-size:8pt; font-weight:600; border:.3mm solid ${acl}; font-family:'${f.skills}',sans-serif; }
.rcert { display:flex; align-items:center; gap:3mm; margin-bottom:3mm; }
.rcert:last-child { margin-bottom:0; }
.rcb { width:12mm; height:12mm; object-fit:contain; border-radius:1mm; background:#fff; border:.3mm solid #e5e7eb; padding:.5mm; flex-shrink:0; }
.rcn { font-size:8.5pt; font-weight:700; color:${fc}; }
.rcm { font-size:7.5pt; color:#6b7280; }
.rcl { font-size:7pt; color:${ac}; text-decoration:none; }
.rlgs { display:flex; flex-wrap:wrap; gap:2mm 6mm; }
.rln { font-size:8pt; font-weight:700; color:${fc}; }
.rll { font-size:7.5pt; color:#6b7280; }
.rints { display:flex; flex-wrap:wrap; gap:1.5mm; }
.rint { padding:.5mm 2mm; background:#f3f4f6; border-radius:100px; font-size:8pt; color:#4b5563; font-weight:500; }

.rdcl { margin-top:auto; padding:6mm 14mm 9mm; border-top:.3mm solid #e5e7eb; }
.layout-creative .rdcl { padding-left:15mm; }
.layout-elegant .rdcl { padding-left:18mm; padding-right:18mm; }
.layout-compact .rdcl { padding:6mm 11mm 7mm; grid-column:1/-1; }
.layout-modern .rdcl { padding:6mm 8mm 7mm; }
.rdclt { font-size:9pt; font-weight:800; color:${ac}; text-transform:uppercase; letter-spacing:1.2px; margin-bottom:2mm; padding-bottom:1mm; border-bottom:.5mm solid ${acl}; font-family:'${f.sectionTitle}',sans-serif; }
.rdctx { font-size:8pt; color:${fc}; line-height:1.65; font-style:italic; margin-bottom:5mm; opacity:.85; font-family:'${f.declaration}',sans-serif; }
.rdcf { display:flex; justify-content:space-between; align-items:flex-end; gap:4mm; }
.rdcm { font-size:7.5pt; color:#6b7280; flex-shrink:0; font-family:'${f.declaration}',sans-serif; }
.rdcs { text-align:center; flex-shrink:0; }
.rdcs img { max-height:16mm; object-fit:contain; display:block; margin:0 auto .5mm; }
.rdcs .sigln { height:.3mm; background:#9ca3af; margin:0 auto 1mm; }
.rdcs .sign { font-size:8.5pt; font-weight:700; color:${fc}; font-family:'${f.declaration}',sans-serif; }
</style>
</head>
<body>
<div class="rpage">
